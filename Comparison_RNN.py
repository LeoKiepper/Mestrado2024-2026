...
from montecarlo_utils import report, NOPLOT, register_metadata, collect_module_variables

NUM_EPOCHS = 1
SEQ_LENGTH = 100
LAG_FEATURE_WINDOW = 500  # seconds. Set to zero to not generate lag features
LAG_FEATURE_EXCLUDE_COLUMNS = ['T_CPU']  # Columns to exclude from lag feature generation
LAG_FEATURE_TARGET_MEM_BYTES = 20e6
HIDDEN_SIZE = 20
NUM_LAYERS = 5
...
TRAINING_SPLIT = 0.8
LEARNING_RATE = 0.001
register_metadata({k:v for k,v in collect_module_variables().items() if k!="NOPLOT"})
...
if __name__ == "__main__":
	...
	if LAG_FEATURE_WINDOW>0: df = generate_lag_features(df, lag_window=LAG_FEATURE_WINDOW, target_lag_memory_bytes=LAG_FEATURE_TARGET_MEM_BYTES, report=False, exclude=LAG_FEATURE_EXCLUDE_COLUMNS)
	...
	#region Training and evaluation for RNN
	...
	def train_rnn_model():
		...
		for epoch in tqdm(range(NUM_EPOCHS), desc="RNN Training"):
			for xb, yb in loader:
				pred = model(xb)
	...
	report(output=rmse, prediction=pred_orig, reference=y[SEQ_LENGTH:], x=df.index[SEQ_LENGTH:], save_metadata=True)