...
MODEL_FILENAME = os.path.join("models","autoencoder_model.pt")
NUM_EPOCHS = 20
SEQ_LENGTH = 50
...
TARGET_COL = 'T_CPU'
FEATURE_COL = [col for col in df.columns]
...
def create_input_sequences(X, seq_length):
	sequences = []
	for i in range(len(X) - seq_length):
		sequences.append(X[i:i+seq_length])
	return torch.tensor(np.array(sequences), dtype=torch.float32)
X_scaled = FEATURE_SCALER.fit_transform(X)
X_seq = create_input_sequences(X_scaled, SEQ_LENGTH)
split_idx = int(TRAINING_SPLIT * len(X_seq))
X_seq_train, X_seq_val = X_seq[:split_idx], X_seq[split_idx:]
train_dataset = TensorDataset(X_seq_train)
val_dataset = TensorDataset(X_seq_val)
loader = DataLoader(train_dataset, batch_size=32, shuffle=True)
val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False)
#endregion

#region Training and evaluation for Autoencoder
class SeqAutoencoder(nn.Module):
	...
	def forward(self, x):
		...
		h_last = h_n[-1]
		seq_len = x.size(1)
		decoder_input = h_last.unsqueeze(1).repeat(1, seq_len, 1)
		decoder_out, _ = self.decoder(decoder_input)
		out = self.output_layer(decoder_out)
		return out
def train_model():
	input_size = X_seq.shape[2]
	model = SeqAutoencoder(input_size=input_size, hidden_size=HIDDEN_SIZE, num_layers=NUM_LAYERS)
	optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)
	criterion = nn.MSELoss()
	from tqdm import tqdm
	for epoch in tqdm(range(NUM_EPOCHS), desc="Autoencoder Training"):
		model.train()
		for batch in loader:
			xb = batch[0]
			pred = model(xb)
		...
		with torch.no_grad():
			...
			for batch in val_loader:
				xb = batch[0]
				pred = model(xb)
				val_loss += criterion(pred, xb).item() * xb.size(0)
...
model.eval()
# with torch.no_grad():
# 	recon = model(X_seq).numpy()

# t_cpu_col_idx = FEATURE_COL.index(TARGET_COL)
# recon_last_tcpu = recon[:, -1, t_cpu_col_idx]

# mean = FEATURE_SCALER.mean_[t_cpu_col_idx]
# scale = FEATURE_SCALER.scale_[t_cpu_col_idx]
# recon_tcpu_orig = recon_last_tcpu * scale + mean

# y_true = y[SEQ_LENGTH:]  # original target aligned with last-step-of-sequence
# plt.figure()
# plt.plot(df.index[SEQ_LENGTH:], y_true, label='Reference')
# plt.plot(df.index[SEQ_LENGTH:], recon_tcpu_orig, label='Reconstruction (last step)')
# from sklearn.metrics import root_mean_squared_error
# rmse = root_mean_squared_error(y_true, recon_tcpu_orig)
